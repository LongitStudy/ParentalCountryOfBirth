...
This code creates three variables from MOPLBINB (Place of birth of mother of 
	new birth on sample date) and three variables from FAPLBINB (place of birth of 
	father of new birth on a sample date) from the NBIR table.
...
*AS ABOVE BUT FOR FATHERS
*YOU DO NOT NEED TO RUN THE LABEL DEFINE COMMANDS AS THEY ARE DEFINED IN CODE ABOVE

tab faplbinb

*count the length of the value minus spaces
gen str shrtmopl=faplbinb
replace shrtmopl=substr(faplbinb,1,4) if substr(faplbinb,5,1)==" "
replace shrtmopl=substr(faplbinb,1,3) if substr(faplbinb,4,1)==" "
gen len=length(shrtmopl)
drop shrtmopl
tab len
...
*create an initial numeric version of moplbinb (won't work for all coding schemes)
gen nummopl=.
replace nummopl=real(substr(faplbinb,1,3)) if substr(faplbinb,5,1)==" "
replace nummopl=real(substr(faplbinb,1,3)) if substr(faplbinb,4,1)==" "
replace nummopl=real(substr(faplbinb,1,5)) if substr(faplbinb,5,1)~=" "

*create a marker variable for those starting '99'
gen code99=0
replace code99=1 if substr(faplbinb,1,2)=="99"

*identify the fifth digit if any (often a space)
gen str last="none"
replace last=substr(faplbinb,5,1)
tab last
...
*else if it begins with 99 and then has a 0 or 1 (Scotland)
replace CX="  1W" if code99==1 & (substr(faplbinb,3,1)=="0" | substr(faplbinb,3,1)=="1") & CX=="    "
replace codschem=1 if code99==1 & (substr(faplbinb,3,1)=="0" | substr(faplbinb,3,1)=="1") & codschem==0

*else if it begins with 99 and then has a 2 (Northern Ireland)
replace CX="  2W" if code99==1 & substr(faplbinb,3,1)=="2" & CX=="    "
replace codschem=1 if code99==1 & substr(faplbinb,3,1)=="2" & codschem==0

*else if it's pre-1992 and begins with 99 ie foreign (knocks off the 99)
replace CX="  "+substr(faplbinb,3,2) if biyranb<1992 & code99==1 & CX=="    "
replace codschem=1 if biyranb<1992 & code99==1 & codschem==0

*else if it's pre-2007 and codschem has been set to 0 (which was the default).
replace CX=faplbinb if biyranb<2007 & codschem==0 & CX=="    "
replace codschem=2 if biyranb<2007 & codschem==0

*else if it's 2007 or later and codschem has been set to 0.
replace CX=faplbinb if biyranb>2006 & codschem==0 & CX=="    "
replace codschem=3 if biyranb>2006 & codschem==0
...
gen str scheme1="na"
replace scheme1=CX if codschem==1
encode scheme1, gen (faplpre92)
label values faplpre92 cob8
replace faplpre92=. if faplpre92==107
lab var faplpre92 "Father's place of birth, for LSMs born 1971-1991"
************************

gen tempcob = faplbinb if codschem==2
*replacing the letters (which are small associated territories etcc) with numbers at beginning
replace tempcob = "1"+substr(tempcob,1,3) if substr(tempcob,4,1) == "A" 
replace tempcob = "2"+substr(tempcob,1,3) if substr(tempcob,4,1) == "B" 
replace tempcob = "3"+substr(tempcob,1,3) if substr(tempcob,4,1) == "C" 
...
gen int faplfr92 = real(tempcob)
replace faplfr92=719 if faplfr92==232 | faplfr92==299
drop tempcob
label values faplfr92 cob01
lab var faplfr92 "Father's place of birth, for LSMs born 1992-2006"
tab faplfr92
**************

*the codes from 2007 on are three numbers, then two spaces (making a length of 5 in all)
gen faplfr07=nummopl if biyranb>2006
lab val faplfr07 zzz
lab var faplfr07 "Father's place of birth, for LSMs born 2007 onward"
***************************************
